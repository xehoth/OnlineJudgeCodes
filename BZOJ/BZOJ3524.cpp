...
} pool[MAXN * 20], *cur = pool, *root[MAXN];
...
inline int query(int l, int r) {
    ...
    register int s = 1, t = n;
    if ((q ? q->cnt : 0) - (p ? p->cnt : 0) <= len) return 0;
    while (s != t) {
        register int mid = s + t >> 1;
        if ((q->lc ? q->lc->cnt : 0) - (p->lc ? p->lc->cnt : 0) > len)
            q = q->lc, p = p->lc, t = mid;
        else if ((q->rc ? q->rc->cnt : 0) - (p->rc ? p->rc->cnt : 0) > len)
...
inline void solve() {
    ...
    for (register int i = 1, x; i <= n; i++)
        read(x), insert(root[i], root[i - 1], 1, n, x);
    for (register int i = 1, l, r; i <= m; i++) {
        read(l), read(r);
        print(query(l, r)), print('\n');
    }
}
}  // namespace SegmentTree
