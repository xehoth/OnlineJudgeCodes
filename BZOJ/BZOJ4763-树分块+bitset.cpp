...
struct BitSet {
    static const int MAXN = size / 64 + ((size % 64) != 0);
    int n;
    ulong a[MAXN + 2];
    ...
    inline int count() {
        register int ret = 0;
        for (register int i = 0; i <= n; i++) ret += __builtin_popcountll(a[i]);
        return ret;
    }
    ...
    inline void reset() {
        memset(a, 0, sizeof(ulong) * (n + 1));
        n = 0;
    }
};

BitSet<MAXV> d[BLOCK_SIZE + 2][BLOCK_SIZE + 2];
...
inline void solve() {
    ...
    for (register int a; m--;) {
        ...
        register int ans2 = tmp.findFirst();
        last = ans1 + ans2;
        io << ans1 << ' ' << ans2 << '\n';
    }
}
