...
inline void solve() {
    ...
    for (register int i = 0; i < n; i++) read(a[i]), sum += a[i];
    for (register int i = 0; i < n; i++) read(b[i]);
    ...
    for (register int i = 1, w; i <= n; i++) {
        w = b[i - 1] - a[i - 1];
        (w > 0) ? (sum += w, addEdge(s, i, w)) : addEdge(i, t, -w);
    }

    for (register int i = n + 1, k, c1, c2; i <= n + m; i++) {
