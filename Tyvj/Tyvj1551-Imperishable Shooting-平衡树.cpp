 ...
 * All rights reserved.
 * 「Tyvj 1360」Imperishable Shooting 22-11-2017
 * 平衡树
 * @author xehoth
 */
#include <bits/stdc++.h>
...
Node::Node(const Point *p) : p(p), size(1) { c[0] = null, c[1] = null; }
...
inline Node **insert(Node *&p, const Point *pt) {
    ...
    if (cmp(p->p, pt))
        ...
        nowAns += p->c[0]->size + 1, res = insert(p->c[1], pt);
    if (p->check()) res = &p;
    return res;
}
...
inline void query() {
    ...
    for (register int i = 1; i <= e; i++)
        io >> poly[i], poly[i] = pos[poly[i] + 1];
    poly[e + 1] = poly[1];
    register int ans = 0, tmp = 0;
    for (register int i = 1; i <= e; i++) {
        if ((pts[poly[i]] - O) * (pts[poly[i + 1]] - O) > 0) {
            ans += f[poly[i]][poly[i + 1]];
        } else {
            ans -= f[poly[i]][poly[i + 1]], tmp++;
        }
    }
    if (ans < 0) ans = -ans, tmp = e - tmp;
    ans = ans - tmp + 1;
    if (ans <= 0)
        io << "Miss!\n";
    else
...
inline void solve() {
    null->c[0] = null->c[1] = null, null->size = 0;
    io >> n;
